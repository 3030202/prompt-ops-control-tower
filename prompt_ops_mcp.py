...
Restores the original read-only Prompt Register MCP surface (Streamable HTTP,
Bearer-protected) and additively exposes meta_agent_compiler as a new tool,
loaded via importlib from the meta-agent-compiler skill directory (hyphenated
path, not a valid Python package name, so it cannot be imported with a normal
dotted import).
"""
...
# --- Additive integration: meta_agent_compiler -----------------------------
# The skill lives under .agents/skills/meta-agent-compiler/, a hyphenated path
# that is not a valid Python package/module name, so it cannot be imported
# with `from .agents.skills.meta_agent_compiler...` (that import is invalid
# and breaks the process at startup). Loading it via importlib by file path
# avoids that problem entirely and keeps this MCP server independent of
...
_meta_agent_compiler_tool = None
if _COMPILER_TOOL_PATH.exists():
    _spec = importlib.util.spec_from_file_location(
        "meta_agent_compiler_tool_module", _COMPILER_TOOL_PATH
    )
    if _spec and _spec.loader:
        _module = importlib.util.module_from_spec(_spec)
        _spec.loader.exec_module(_module)
        _meta_agent_compiler_tool = _module.MetaAgentCompilerTool(idempotency_cache={})


if _meta_agent_compiler_tool is not None:

    @mcp.tool()
    async def meta_agent_compiler(
        trace_context: Annotated[dict, Field(description="W3C Trace Context: trace_id, span_id, parent_span_id, trace_flags.")],
        task_metadata: Annotated[dict, Field(description="task_id, attempt_number, max_attempts.")],
        payload: Annotated[dict, Field(description="raw_task_description and optional hints.")],
        circuit: Annotated[dict | None, Field(description="Optional circuit breaker state.")] = None,
        error_diff: Annotated[dict | None, Field(description="Optional error context for retries.")] = None,
    ) -> dict:
        """Compile an unstructured task description into a graph topology, JSON Schema contracts, and a deterministic system prompt."""
        return await _meta_agent_compiler_tool(
